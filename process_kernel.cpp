...
void process(uint8_t readbuf[READ_SIZE], off_t& num_read, uint16_t channels[NUM_CHANNELS]) {
    ...
    for (int comp = 0; comp < trigRecHeader->num_requested_components; ++comp) {
        ...
        for (int iframe = 0; iframe < num_frames; ++iframe) {
            ...
            for (int ich = 0; ich < NUM_CHANNELS; ++ich) {
                auto chanval = frame->get_channel(ich);
                if (chanval > channels[ich]) {
                    channels[ich] = chanval;
                }
            }
